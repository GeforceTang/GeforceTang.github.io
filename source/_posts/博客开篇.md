...
有没有发现build失败了，哈哈，官方文档又埋坑了。原因是没有执行install，所有插件都没有执行hexo d肯定不行。解决方法在`script`上增加命令：
```yml
install:
  - npm install
```

查问题过程还偶然发现很不错的一个博客 https://blog.i1hao.com/2018/09/01/hexo-and-githubpages-best-practices/
